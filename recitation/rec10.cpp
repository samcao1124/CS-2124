...
int main() {
    ...
    vector<PrintedMaterial*> ptrToPm;
    ptrToPm.push_back(&text);
    ptrToPm.push_back(&novel);
    ptrToPm.push_back(&mag);
    for (size_t index=0; index< ptrToPm.size(); ++ index){
        ptrToPm[index] -> displayNumPages();
    }
